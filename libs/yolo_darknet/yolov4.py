...
def get_output_v4(detections, image_h, image_w, input_size=608):
    list_bboxes = []
    list_scores = []
    list_labels = []
    for label, confidence, bbox in detections:
        x, y, w, h = bbox
        # convert center to coner point
        x = x - w/2
        y = y - h/2

        # convert to raw size
        x *= image_w / input_size
        w *= image_w / input_size
        y *= image_h / input_size
        h *= image_h / input_size

        left, top, right, bottom = int(x), int(y), int(x+w), int(y+h)

        list_bboxes.append([left, top, right, bottom])
